...
// Add RESTRICT_FOLDER_ACCESS for folders
commands.RESTRICT_FOLDER_ACCESS = function (meta, args) {
    if (!Array.isArray(args) || typeof(args[0]) !== 'boolean') {
        throw new Error('INVALID_STATE');
    }

    const restricted = args[0];
    if (meta.folderRestricted === restricted) { return false; }

    meta.folderRestricted = restricted;

    if (restricted && !Array.isArray(meta.allowed)) {
        meta.allowed = [];
    }
...
// Add users/teams to the folder's `allowed` list
commands.ADD_FOLDER_ALLOWED = function (meta, args) {
    if (!Array.isArray(args)) {
        throw new Error('INVALID_STATE');
    }

    // Deduplicate and add users/teams to the `allowed` list
    meta.allowed = deduplicate((meta.allowed || []).concat(args));
